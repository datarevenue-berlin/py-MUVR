...
from omigami.recursive_feature_eliminator import (
    ...
    RecursiveFeatureEliminationResults,
)
...
def inner_loop_results_2():
    return [
        FeatureEvaluationResults(
            ranks=FeatureRanks(features=[1], ranks=[1]), test_score=0.4,
        ),
        FeatureEvaluationResults(
            ranks=FeatureRanks(features=[1], ranks=[1]), test_score=0.4,
        ),
...
def test_feature_eliminator(feature_evaluator):
    rfe = RecursiveFeatureEliminator(
        feature_evaluator, dropout_rate=0.1, robust_minimum=0.05
    )
    assert rfe
...
def test_run(feature_evaluator):
    rfe = RecursiveFeatureEliminator(
        feature_evaluator, dropout_rate=0.5, robust_minimum=0.05
    )
