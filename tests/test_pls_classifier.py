...
from omigami.pls_classifier import PLSClassifier
...
def test_fit(pls_classifier, mosquito):
    ...
    assert fitted_classifier.coef_
    assert len(fitted_classifier.coef_) == 1
    assert len(fitted_classifier.coef_[0]) == mosquito.X.shape[1]
